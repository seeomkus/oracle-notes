...
- Oracle Linux 6
- Original SID: `GEN21` (renamed to lowercase `gen21`)
...
Change the `ORACLE_SID` used to connect to and manage an existing, already-running database — in this case only a case change (`GEN21` → `gen21`) — without rebuilding the database.
...
Oracle resolves which instance to attach to by matching the shell's `ORACLE_SID` against a parameter file named `spfile<SID>.ora` (or `init<SID>.ora`) in `$ORACLE_HOME/dbs`. Because `INSTANCE_NAME` (and by default `SERVICE_NAMES`) inside the SPFILE was itself already set based on the original SID value, simply copying the SPFILE to a new filename and shutting down/starting up under the new `ORACLE_SID` is enough to make Oracle instantiate under the new name — as shown in this session, where the listener automatically began advertising the service as `gen21` after restart. If you need a true full database rename (different `DB_NAME`, not just instance identity), use the `DBNEWID` (`nid`) utility instead — see the note at the end of this document.

## Troubleshooting / Procedure Flow

```mermaid
flowchart TD
    A[Set ORACLE_HOME, ORACLE_SID=GEN21, PATH] --> B[Copy spfileGEN21.ora to spfilegen21.ora]
    B --> C[sqlplus / as sysdba: SHUTDOWN IMMEDIATE]
    C --> D[Database closed, dismounted, instance shut down]
    D --> E[export ORACLE_SID=gen21]
...
```bash
cp $ORACLE_HOME/dbs/spfileGEN21.ora $ORACLE_HOME/dbs/spfilegen21.ora
```

This creates the parameter file that Oracle will look for once `ORACLE_SID` is changed to `gen21`. The original `spfileGEN21.ora` is left in place as a backup/rollback point.
...
```
Services Summary...
Service "gen21" has 1 instance(s).
  Instance "gen21", status READY, has 1 handler(s) for this service...
```
...
1. **`/etc/oratab`** — update the entry from `GEN21:/u01/app/oracle/product/11g/dbhome_1:Y` to `gen21:/u01/app/oracle/product/11g/dbhome_1:Y` (Oracle Linux SID entries are case-sensitive; leaving both can cause `dbstart`/`dbshut` scripts to reference a stale value).
2. **Shell profile / login scripts** (`~/.bash_profile`, `~/.bashrc`, or any `oraenv`-based scripts) — update any hardcoded `ORACLE_SID=GEN21` so future logins default to the correct SID.
3. **Password file** — if a password file exists (`orapwGEN21`), copy/rename it to match: `cp $ORACLE_HOME/dbs/orapwGEN21 $ORACLE_HOME/dbs/orapwgen21`. Without this, remote SYSDBA connections using password-file authentication may fail to find credentials under the new SID.
...
7. **Remove or archive the old SPFILE** (`spfileGEN21.ora`) only after confirming the new SID has been stable through at least one full restart cycle — keep it temporarily as a rollback option.
8. **Confirm `INSTANCE_NAME` and `SERVICE_NAMES` parameters** explicitly, rather than relying on defaults, if client connect strings depend on a specific service name:
   ...
   If either still reflects the old naming and application connect strings depend on it, set them explicitly with `ALTER SYSTEM SET service_names='gen21' SCOPE=BOTH;`.
...
```bash
nid TARGET=sys/<password>@gen21 DBNAME=newdbname
```
